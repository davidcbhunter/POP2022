...
class Trump:
    ...
    def Flip(self):
        ...
        if self.is_showing_face == False:
            ...
            #we need to update to make the changes show
            self.button.update()
...
def CheckChoices():
    ...
    if card_dictionary[first_choice].trump_name == card_dictionary[second_choice].trump_name:
        print("Nice")
        player_scores[player_index] += points_per_match
        label_list[player_index].configure(text = "Player " + str(player_index + 1) \
                                           + " Score: " + str(player_scores[player_index]))
        label_list[player_index].update()
        ...
        time.sleep(3) #wait a short time before we flip
        card_dictionary[first_choice].Flip()
        card_dictionary[second_choice].Flip()

    first_choice = -1
    second_choice = -1
    label_list[player_index+2].place_forget()
    player_index += 1
    if player_index > 1:
        player_index = 0
    if IsGameOver():
        ...
        if player_index == 0:
            label_list[player_index+2].place(x=0,y=20)
        else:
            label_list[player_index+2].place(x=924,y=20)


def IsGameOver():
    return len(card_dictionary) == 0
...
label = tk.Label(root,text = "Player 1 Score: " + str(player_scores[0]))
label.place(x = 0, y = 0)
label_list.append(label)

label = tk.Label(root,text = "Player 2 Score: " + str(player_scores[1]))
